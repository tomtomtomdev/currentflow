# IDX Smart-Money Screener & Flow Terminal — Locked Specification v1.7
...
**Changelog — v1.7 (2026-07-15):** added **LD-14 (Pattern catalog — event-cadence base rates)** — a new *presentation category* introduced by the subordinate spec [`PATTERN-CATALOG-SPEC.md`](PATTERN-CATALOG-SPEC.md) (windows/seams from [`REGIME.md`](REGIME.md)). Bandarmology/Wyckoff folklore is converted into falsifiable, provenance-tagged pattern classes measured against a **point-in-time universe** (slice 17), each carrying `n`, hit rate, **Wilson 90% CI**, its **unconditional null**, confound strata, and an out-of-sample split at the holdout seam. *Reason:* trade-cadence validation is structurally starved (~1 armed trade/yr/32 names — the LD-11/LD-12 deadlock); a pattern instance needs only a flagged date + a forward outcome, so event cadence yields **hundreds** of observations where the trade path yields single digits — giving §4/§5 an empirical footing **without touching weights** (the walk-forward optimizer stays the sole writer) or any gate. **Hard boundary — presentation only, nothing in the engine changes:** base rates are *measurements* (like a z-score), confined to a **dedicated catalog/research view** under rules **P1–P4** — they NEVER render on a live candidate row, the pipeline, the ARMED rail, or an evidence tab (P1); carry **no buy/sell verb and no cross-pattern composite**, and are never multiplied into SMS or any displayed number (P2); attaching a pattern's stats to a **live name is a claim** and goes through the standard RULE B path (a dedicated `ValidationLedger` lane), not the catalog (P3); and small-n renders the interval only, a window with no instances renders "no instances", never 0% (P4). **No decision (LD-1…LD-13), no SMS weight, no `ARMED@70` threshold, no veto, no phase-gate (RULE A), and no presentation-gate (RULE B) behaviour changed** — LD-14 *extends* RULE B with a fourth presentation category, it does not relax it. Companion infra: **slice 17** clamps every historical computation to `regime_start(track)` (REGIME.md) and reconstructs survivorship-honest per-day universes (`pit_universe`) — cache-only, RULE A/B untouched, **no spec bump** for slice 17 itself. See PROGRESS.md decisions log (2026-07-15). *(Note: the plan draft proposed "LD-13, v1.6"; that number/version was already consumed by the detection-enrichment bump below, so the pattern catalog is renumbered LD-14, v1.7.)*

**Changelog — v1.6 (2026-07-15):** added **LD-13 (Detection enrichment from already-held data)** — three backtestable smart-money signals sourced from data the system *already fetches or can derive from stored OHLCV*, grounded in the **Bandarmology / Wyckoff 2.0 / VPA** frameworks: **(a) Institutional-Ownership Delta** — the KSEI shareholder-composition feed (already ingested + stored, today consumed **only by the UI**) wired into detection as a slow-money accumulation confirmation (rising institutional/foreign ownership across the range confirms Composite-Man accumulation; ownership *falling* while price is marked flat/up is a distribution tell); **(b) VPA bar-character** (No-Demand / No-Supply / Absorption) — the close-position-within-the-spread that Coulling's VPA turns on, **absent today** (the divergence spine sees only volume vs \|Δclose\|, never *where* the bar closed); **(c) approximate Volume Profile** (POC / VAH / VAL / HVN / LVN) — Wyckoff 2.0's signature addition, computed as a **daily-bar volume-at-price approximation** (fidelity caveat: true VP needs intraday depth the system does not backtest — the approximation never claims more precision than daily bars support). **Hard boundary — nothing in the running score changes:** all three enter §4 as **candidate components pinned at weight 0** (inert, exactly the LD-1 Track-B `foreign_flow=0` pin); only the walk-forward Sharpe optimizer may raise a weight (never hand-edited — §4), and only after RULE B forward-paper earns it. RULE A's C/D gate *decision rule* is unchanged — (b)/(c) add corroboration to the phase detectors, never a new gate. Each signal ships **first as a RULE-B-clean observation module** (raw signal, no number) per §9. **Live-only footprints stay fenced out of the score:** L2 order-book absorption and running-trade "hajar kanan" (the frameworks' direct tape tells) can't share the backtest fill engine (§13 parity), so they remain observation-only and are never a validated-score input (LD-5 posture unchanged). §4.1/§9/§11/§13 updated. **No decision (LD-1…LD-12), no SMS *active* weight, no `ARMED@70` threshold, no veto, no phase-gate (RULE A), and no presentation-gate (RULE B) behavior changed.** See PROGRESS.md decisions log (2026-07-15).
...
| **LD-14** *(v1.7)* | Statistical evidence starved at trade cadence | **Pattern catalog — event-cadence base rates, presentation-only.** Bandarmology/Wyckoff folklore → falsifiable pattern classes measured over a point-in-time universe (slice 17): per (pattern, track, horizon) `n`, hit rate, Wilson 90% CI, the unconditional null, confounds, and an OOS split at the holdout seam. Confined to a dedicated catalog view under **P1–P4** (never on a live candidate/pipeline/rail/evidence surface; no buy/sell verb, no composite, never multiplied into SMS; attaching to a live name is RULE-B-gated; small-n renders interval-only). See [`PATTERN-CATALOG-SPEC.md`](PATTERN-CATALOG-SPEC.md). | Trade-cadence validation is starved (~1 armed trade/yr/32 names — the LD-11/12 deadlock). A pattern instance needs only a flag date + forward outcome, so event cadence gives §4/§5 an empirical footing at hundreds-of-observations scale **without touching weights** (optimizer stays sole writer) or any gate. A new displayed-number category warrants a documented bump; it *extends* RULE B (a 4th presentation category), never relaxes it. |
| **LD-13** *(v1.6)* | Detection enrichment vs data already held | **Wire the data we already have.** Three backtestable signals from already-fetched / OHLCV-derivable data, per Bandarmology / Wyckoff 2.0 / VPA: (a) **KSEI Institutional-Ownership Delta** (fetched + stored, today UI-only) as slow-money accumulation confirmation + a distribution-veto refinement; (b) **VPA bar-character** (No-Demand / No-Supply / Absorption via close-position-in-spread — absent today); (c) **approximate Volume Profile** (POC/VAH/VAL/HVN/LVN from daily bars). All enter §4 **pinned at weight 0** (inert; optimizer-only raise, RULE B-gated); each ships first as a RULE-B-clean observation module. RULE A decision rule unchanged; live-only tape (L2 absorption, running-trade) stays observation-only (backtest-parity fence, LD-5). See §4.1. | The core (divergence spine + phase gate + broker persistence + foreign flow) is framework-faithful, but three signals the frameworks weight heavily are unused: KSEI ownership is fetched yet ignored by detection; VPA's close-in-spread is absent (the spine sees only \|Δclose\|); Wyckoff 2.0's Volume Profile doesn't exist. Adding them **inert (weight 0)** enriches the §4 menu with **zero running-score change** until the optimizer + forward paper earn each one — the honest, no-hand-tuning path (RULE B / §4). |
...
### 4.1 Candidate detection components (LD-13, v1.6) — validation-pending, weight 0

Three signals extend the §4 menu, each grounded in the loaded frameworks (Bandarmology / Wyckoff 2.0 / VPA). **They are inert until earned:** every candidate enters the SMS simplex **pinned at weight 0** (the LD-1 Track-B `foreign_flow=0` pattern) and only the walk-forward Sharpe optimizer (`ml/optimizer.py`) may raise it — **never a hand-edit** (§4) — and only once RULE B forward-paper validates it. Until then the running score is byte-for-byte unchanged, and each signal is visible **only as a RULE-B-clean observation module** (§9).

| Candidate | Framework | Source (already held / derivable) | Enters as |
|---|---|---|---|
| **Institutional-Ownership Delta** | Bandarmology §2/§10 (institutions = the real, slow bandar); Wyckoff CM | KSEI shareholder composition — **already fetched + stored** (`ksei_ownership`), today UI-only | SMS confirmation sub-signal (weight 0) **and** a distribution-veto refinement (ownership falling while price flat/up) |
| **VPA bar-character** (No-Demand / No-Supply / Absorption) | Coulling VPA; Wyckoff effort-vs-result | Stored OHLCV — close position within the bar spread (absent today; divergence sees only volume vs \|Δclose\|) | phase-detector corroboration (Spring/UTAD effort-vs-result) **and** a candidate refinement to the divergence spine (weight 0) |
| **Approximate Volume Profile** (POC / VAH / VAL / HVN / LVN) | Wyckoff 2.0 (Villahermosa) — VP is its signature addition | Stored daily OHLCV — daily-bar volume-at-price approximation | phase-context corroboration (Spring@VAL / UTAD@VAH / LPS@POC); candidate `phase_bonus` enrichment (weight 0) |

**Fidelity honesty (a `missing ≠ zero` analogue):** the Volume Profile is a *daily-bar approximation* — true POC / value-area precision needs the intraday depth the system does not backtest. It must never render or imply more precision than daily bars support.

**RULE A unchanged:** (b) and (c) add *corroboration* to the phase detectors; the C/D tradeability-gate decision rule is not altered. **Live-only footprints stay fenced (LD-5, §13):** L2 order-book absorption and running-trade "hajar kanan" — the frameworks' direct tape tells — cannot share the backtest fill engine, so they remain observation-only and are **never** inputs to the validated score.
...
- **Institutional-Ownership Delta** *(LD-13, v1.6)* — KSEI shareholder-composition change over the accumulation window as a slow-money confirmation (rising inst/foreign ownership ↔ Composite-Man accumulation; falling-while-marked-up ↔ distribution). Promotes the Foreign Flow dashboard's KSEI overlay from display into a **categorical** observation. No number.
- **VPA Bar-Character** *(LD-13, v1.6)* — per-bar No-Demand / No-Supply / Absorption / effort-vs-result read from close-position-within-the-spread (Coulling). Categorical severities, no number.
- **Volume Profile** *(LD-13, v1.6)* — approximate daily-bar volume-at-price (POC/VAH/VAL/HVN/LVN) as a chart overlay for the phase / replay views, with the daily-bar fidelity caveat surfaced. Levels are measurements, never a score.
...
- **Detection enrichment (v1.6, LD-13)** — three vertical slices (data → signal → view → test) wiring the already-held / OHLCV-derivable signals of §4.1: **KSEI Institutional-Ownership Delta**, **VPA bar-character**, and **approximate Volume Profile**. Each ships a RULE-B-clean observation module and registers a §4 candidate component **pinned at weight 0**; the optimizer raises a weight only after forward paper earns it (RULE B). **No running-score change on landing.**
...
- [x] **(v1.4)** Fast Mode enters an ARMED name with no trigger and R:R < 2:1 (LD-11), while the standard [6] path still skips it; the Fast-Mode buy exits via the same §8 ladder; its trades promote only the `fast_mode` lane (not `sms`/`ai_ranking`/`daily_top`) and its aggregate number stays withheld until validated (RULE B).
- [x] **(v1.5)** Haste Mode enters a `WATCH` name (phase C/D, no veto, SMS < 70) with no trigger/R:R (LD-12), while Fast Mode does **not** enter it and the standard [6] path skips it; a `GATE_REJECTED` (non-C/D) or `VETOED` name is **never** entered by either auto-trader (RULE A / §5 firewall); the Haste-Mode buy exits via the same §8 ladder; its trades promote only the `haste_mode` lane (not `fast_mode`/`sms`/`ai_ranking`/`daily_top`) and its aggregate number stays withheld until validated (RULE B).
- [ ] **(v1.6)** LD-13 candidate components (KSEI ownership delta, VPA bar-character, Volume Profile) enter §4 at **weight 0** and are inert — the running SMS is **unchanged on landing**; only the walk-forward optimizer raises a weight, and only post-validation (RULE B); each ships as an observation module that renders **no number**; the Volume Profile never implies sub-daily precision; L2 absorption / running-trade are **never** wired into the validated score (backtest-parity fence).
- [x] **(v1.7)** LD-14 pattern-catalog base rates render **only** in the dedicated catalog view (P1) — a grep-style firewall test confirms they appear on no candidate/pipeline/rail/evidence render path; small-n renders **interval-only** and an empty window renders "no instances" not 0% (P4); the **unconditional null travels with every stored rate** (rejected at write otherwise, §5.4); the scanner refuses a **non-PIT** universe source (§5.1); instances are look-ahead-safe and **terminal outcomes are counted, never dropped** (§5.6); the estimator is **deterministic** (byte-identical re-run); **no SMS weight is written** by any catalog code path (optimizer remains sole writer).
...
*v1.7 (2026-07-15): LD-14 pattern catalog — event-cadence base rates (n / Wilson 90% CI / unconditional null / OOS split) over a point-in-time universe, confined to a dedicated catalog view under P1–P4 (never on live candidate/pipeline/rail/evidence surfaces; attachment to a live name is RULE-B-gated). Subordinate spec PATTERN-CATALOG-SPEC.md; windows from REGIME.md; §1/§13 updated. Companion slice 17 (regime clamp + PIT universe) is cache-only, no bump. Running score / RULE A / RULE B behaviour unchanged (RULE B extended, not relaxed). Renumbered from the plan draft's "LD-13, v1.6" (already taken).*
*v1.6 (2026-07-15): LD-13 detection enrichment — KSEI Institutional-Ownership Delta, VPA bar-character, approximate Volume Profile; added as §4.1 candidate components pinned at weight 0 (optimizer-only, RULE B-gated), each shipping first as an observation module; §1/§4.1/§9/§11/§13 updated; running score / RULE A / RULE B unchanged; live-only tape (L2 / running-trade) fenced out of the score.*
